...
int main(){
  ...
  for(int d=2; d<7;d++){
	...
	for(int k=0; k<Ngenerators; k++){
	  ...
	  if(std::abs(ACommutator(v1,v2)*v3)>1e-15){
	    if(!set){
	      file >> in; 
	      file >> jn; 
	      file >> kn;	    
	    ...
	    if( i!=in || k!=kn ||k!=kn){
	      ...
	      if(val-(ACommutator(v1,v2)*v3)/4.0>1e-4){
		ok=false;
		std::cout << i << "  " << j << "  " << k << "  " << val-(ACommutator(v2,v3)*v1)/4.0 << "  " << val << std::endl;
	      }else{
		set=false;
	      }
	    }
    ...
    if(!ok)
      std::cout << "anti-commutator wrong, dim: " << dim << std::endl; 
  }
